...
- 2026-07-04: `code-reviewer` サブエージェントのレビューを反映（bin の出力切り捨て潜在バグを `process.exitCode` で修正、JSON import を設計正本へ整合、フラグ先勝ち short-circuit を test で固定、bin の I/O 結線検出を追加、パッケージ bunfig を hardening）。ローカルゲート全 Green で PR #11 を作成。
- 2026-07-04: 初回 CI が `bun install --frozen-lockfile` で失敗（`lockfile had changes`）。原因は `bun add` を workspace パッケージ作成前に実行し、lockfile に `annai-term` エントリが未記録だったこと。`bun install` で再生成して追随、CI Green。

#### 振り返り

- **問題**: ローカルゲートは全て Green だったが、初回 CI が `--frozen-lockfile` で失敗した。
- **根本原因**: 依存追加（`bun add`）を新しい workspace パッケージの `package.json` 作成より前に実行したため、lockfile に workspace エントリが記録されなかった。ローカルの `bun test` / `typecheck` / `make before-commit` は frozen install を経由しないので露見せず、frozen を強制する CI で初めて顕在化した。
- **予防策**: 新しい workspace パッケージを足したら、コミット前に `bun install --ignore-scripts` を一度走らせて lockfile を追随させる。順序としては「パッケージの `package.json` を先に作る → 依存を足す」。ローカルの完了確認に frozen install（`make install_ci` 相当）を含めると CI との差を先に潰せる。
